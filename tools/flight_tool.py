import os 
import re # Regular Expressions -> for searching, extracting, and cleaning text using patterns
import certifi # Check certificate of the visited website
import airportsdata
import pycountry
import requests # Lets Python make HTTP requests to APIs.
...
# To clean the whole query by user for agent to understand
...
# Read a user's sentence and find every country or city mentioned in it
...
# Read a user's natural language query and determine:
...
# Take one flight returned by the AviationStack API and format it into a nice, readable string
...
# Take a user's query → understand the route → call the AviationStack API → return nicely formatted flight information
